        ...
        while (scan.hasNextLine()) {
            ...
            for (int j = 0; j < ticket.length; j++) {
                ...
                for (int i = 0; i < ranges.length; i++) {
                    if ((Integer.parseInt(ticket[j]) >= ranges[i][0][0]
                            && Integer.parseInt(ticket[j]) <= ranges[i][0][1])
                            || (Integer.parseInt(ticket[j]) >= ranges[i][1][0]
                                    && Integer.parseInt(ticket[j]) <= ranges[i][1][1])) {
                        valid = true;
                        break;
                    }
                }
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (int j = 0; j < size; j++) {
                ...
                for (String cosa : tickets) {
                    ...
                    // System.out.println(value+" first["+ranges[0][0][0]+", "+ranges[0][0][1]+"]"+"
                    // second["+ranges[0][1][0]+", "+ranges[0][1][1]+"]");
